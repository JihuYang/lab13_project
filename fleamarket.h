...
>>>>>>> 6e28df13e58bb5340c74d8e8644017aaa6e8b243
=======
void listProduct(Product *p, int count); //제품 나열을 하기 위한 함수
...
int ID(Data* data, int ID_count); // 최초 접속시 아이디 소유 여부에 따라 로그인이나 회원가입을 진행하도록하는 함수
void SaveData_Member(Data* data,int id_sub); //회원정보 데이터를 저장하기 위한 함수
int LoadData_Member(Data* data); //회원정보 데이터를 불러오기 위한 함수
